...
class TopPartOneView: UIView {
    ...
    @objc fileprivate func mainViewClick() {
    ...
    ///设置更多的view
    func modelSetAsMore(_ image:String,id:Int,title:String) {
    ...
    class func loadTopPartOneViewFromXib(_ index:CGFloat,ySet:CGFloat) -> TopPartOneView {
        let view = Bundle.main.loadNibNamed("TopPartOneView", owner: nil, options: nil)?.last as? TopPartOneView
        view?.frame = CGRect(x: index * (AppWidth / 4), y: ySet, width: (AppWidth / 4), height: 100)
        return view!
    }
